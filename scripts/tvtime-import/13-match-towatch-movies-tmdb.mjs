// Matches recovered "want to watch" movies (data/towatch-movies.json) to
// TMDB ids using title + release year, same approach as the watched-movies
// matcher.
...
async function main() {
  const { movies } = JSON.parse(fs.readFileSync(path.join(dataDir, "towatch-movies.json"), "utf8"));
  ...
  for (const m of movies) {
    ...
    if (candidates.length === 0 && year) candidates = await searchMovie(m.title, null);
    await sleep(120);
  ...
  fs.writeFileSync(path.join(dataDir, "towatch-movies-match-report.json"), JSON.stringify({ movies: results }, null, 2));
