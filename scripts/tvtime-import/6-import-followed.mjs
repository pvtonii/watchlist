// Adds resolved-followed.json shows to library_items with status "watchlist"
// (Want to Watch). Dry-run by default — pass --commit to actually write.
...
async function main() {
  const { shows } = JSON.parse(
    fs.readFileSync(path.join(dataDir, "resolved-followed.json"), "utf8")
  );

  console.log(`${shows.length} shows to add as "Want to Watch".`);
