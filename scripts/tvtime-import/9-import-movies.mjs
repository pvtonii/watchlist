// Adds recovered movies to library_items as "completed", using the real
// watched date for both created_at/updated_at (the library page reads
// updated_at as the "Watched <date>" label for completed items).
...
async function main() {
  ...
  for (const m of resolved) {
    ...
    const label = `[${done}/${resolved.length}] ${m.title} -> "${m.chosenTitle}" (tmdb ${m.chosenTmdbId}), watched ${m.watchedAt.slice(0, 10)}`;
